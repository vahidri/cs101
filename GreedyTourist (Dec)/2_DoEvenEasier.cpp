// Even Better!
	///we don't need to make the 'Parted' array, with a function(actually I did it in earlier version, cuz before I make this, I started with a Graph Partitioner program => Function!)
...
int cvc(short int ** ajc_mat); //Connected Vertexes Count(er)
...
cout<<"The Process:\n";
	while(q!=n-1) //steps(till it becomes a tree)
	{
		...
		out[Va][Vb]=1;
		out[Vb][Va]=1;
		parted_replace(parted[Vb],parted[Va]);
		Cost+=m;
